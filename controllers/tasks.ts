...
export const getTasks: Deno.ServeHandler = async () => {
    try {
        const dbRes = await db.query('SELECT id, title, description, status, dueDate FROM tasks');
...
export const addTask: Deno.ServeHandler = async (req) => {
    try {
        ...
        return new Response(JSON.stringify({ message: "Task added successfully", task: newTask }), {
            status: 201,
            headers: {
                'Content-Type': 'application/json',
            },
...
export const deleteTask: Deno.ServeHandler = (req) => {
    try {
